...
(function() {
  ...
  window.StageHandlers.quantizer = {
    ...
    renderTheory: function(stage, params, toLatexNumber, SignalData) {
      ...
      formulas += `<div class="formula-preview"><span>Шум квантования по методичке (формулы 7–10)</span>\\[ \\varepsilon_{\\text{кв}}^2=P_x-2B_{xv}+P_v=P_x(1-2\\gamma)+P_y \\]</div>`;
      ...
      formulas += `<div class="formula-preview"><span>Подстановка чисел варианта</span>\\[ \\varepsilon_{\\text{кв}}^2=${toLatexNumber(Px.toFixed(4))}(1-2\\cdot${toLatexNumber(gamma.toFixed(4))})+${toLatexNumber(Py.toFixed(4))}=${toLatexNumber(eps.toFixed(4))}\\text{ В}^2 \\]</div>`;
      formulas += `<div class="formula-preview"><span>Приближённая оценка (для сравнения)</span>\\[ \\varepsilon_{\\text{кв},\\text{прибл}}^2\\approx\\frac{\\Delta U^2}{12}=\\frac{${toLatexNumber(dU.toFixed(3))}^2}{12}\\approx ${toLatexNumber(epsApprox.toFixed(4))}\\text{ В}^2 \\]</div>`;
      formulas += `<div class="stage-panel__info-box"><strong>Связь с графиком:</strong><br>Серые пунктирные линии — пороги \\(u_i\\), зеленые горизонтали — выходные уровни \\(v_j\\). Столбцы \\(p_j\\) и линия \\(F_j\\) пересчитываются из гауссовского закона при каждом изменении \\(P_g\\). Основной расчёт \\(\\varepsilon_{\\text{кв}}^2\\) использует \\(P_x\\), \\(P_y\\) и \\(\\gamma\\) по формулам 7–10 методички.</div>`;
